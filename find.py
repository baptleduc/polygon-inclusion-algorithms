...
class Find:
    ...
    def area_local_vision(
        polygones, algo, display_center_point=False, display_fast_voxel=False
    ):
        """
        ...
        inclusions_possibles_abscisse = Find.__potential_inclusions(
            liste_abscisse, len(polygones)
        )
        inclusions_possibles_ordonne = Find.__potential_inclusions(
            liste_ordonne, len(polygones)
        )
        # intersection of results
        inclusions_possibles = Find.__intersection(
            inclusions_possibles_ordonne, inclusions_possibles_abscisse
        )
        tab_grid = [None for _ in range(len(polygones))]
        inclusions: list = [-1 for _ in range(len(polygones))]
        # state 2
        ...
        for polygone1 in range(len(inclusions_possibles)):
            for polygone2 in inclusions_possibles[polygone1]:
                ...
                if algo == "grid":
                    if tab_grid[polygone2] == None:
                        tab_grid[polygone2] = GridPointInPolygon(
                            polygones[polygone2], 400
                        )
                        # display for debug
                        affiche_fast_voxel = []
